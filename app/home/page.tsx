...
export default function StopMotion() {
  ...
  useEffect(() => {
    if (!isLoaded || !productPageOn) return;
  ...
  useEffect(() => {
    ...
    const handleProductPageClick = (e: MouseEvent) => {
      ...
      if (
        ...
        // input, select, textarea, [role="button"], .pre-order-container
      ) {
        return;
      }
      ...
      if (currentFrame > 0 && currentFrame < 230) {
        targetFrame = 0;
      } else if (currentFrame > 230 && currentFrame < 410) {
        targetFrame = 140;
      } else if (currentFrame > 410 && currentFrame < 510) {
        targetFrame = 260;
